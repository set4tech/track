export function generateClientScripts(totalDecisions, decisionsWithTags) {
  return `
    ...
    function buildTimeline() {
      ...
      timelineContainer.innerHTML = '';
      quotePanel.innerHTML = '<h3 class="quote-heading">Evolution</h3>';
    ...
    function toggleTag(button) {
      ...
      if (selectedTags.length === 0) {
        history.replaceState(null, '', '/api/index');
        switchToGrid();
      } else {
        ...
        history.replaceState(null, '', \`/api/index?\${params.toString()}\`);
        switchToTimeline();
      }
    }
    ...
    function filterByPriority(priority) {
      ...
      const newUrl = \`/api/index?\${queryString}\`;
