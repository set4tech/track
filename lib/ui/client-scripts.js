export function generateClientScripts(totalDecisions, decisionsWithTags) {
  return `
    ...
    function expandDecision(decision, event) {
      // Get the click position relative to the decision element
      ...
      // Set transform origin before expanding
      decision.style.transformOrigin = `${originX}% ${originY}%`;
      
      // Start with scale 0 from the click point
      decision.style.transform = 'scale(0)';
      ...
      // Enable transition and animate to full size
      decision.style.transition = 'all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
      decision.style.transform = 'translate(-50%, -50%) scale(1)';
    }
    ...
    function collapseDecision(decision) {
      ...
      setTimeout(() => {
        ...
        decision.style.transformOrigin = '';
