...
function getDecisionMakerColor(name) {
  ...
  const lightness = 35 + (Math.abs(hash) % 15); // 35-50% for good readability
  ...
  const html = `
    ...
    <body>
      ...
      <div class="stats-section">
        <div class="stats">
          <div class="stat" onclick="clearPriorityFilter()">
            <strong>${decisionsWithTags.length}</strong> confirmed decisions
          </div>
          <div class="stat${config.priorityFilter === 'critical' ? ' active' : ''}" onclick="filterByPriority('critical')">
            <strong>${decisionsWithTags.filter((d) => d.priority === 'critical').length}</strong> critical
          </div>
          <div class="stat${config.priorityFilter === 'high' ? ' active' : ''}" onclick="filterByPriority('high')">
            <strong>${decisionsWithTags.filter((d) => d.priority === 'high').length}</strong> high priority
          </div>
          <div class="stat" onclick="showDecisionMakers()">
