...
// Google OAuth client (only initialize if configured)
let googleClient = null;
if (AUTH_CONFIG.google.clientId) {
  googleClient = new OAuth2Client(AUTH_CONFIG.google.clientId);
}
...
export async function requireAuth(req, res) {
  ...
  const token = cookies[AUTH_CONFIG.jwt.cookieName];
  ...
  try {
    ...
    // If users table doesn't exist, just return unauthenticated
