...
export function generateAuthHTML(csrfToken) {
  return `
    ...
    <script>
      ...
      document.cookie = '${AUTH_CONFIG.csrf.cookieName}=${csrfToken}; path=/; samesite=lax${AUTH_CONFIG.jwt.cookieOptions.secure ? '; secure' : ''}';
  ...
  if (user) {
    ...
    return `
      <div style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
        ...
        `}
        <span style="color: var(--muted-foreground);">${user.name || user.email}</span>
        <button onclick="signOut()" style="
          padding: 4px 8px;
          background: transparent; border: 1px solid var(--border); border-radius: 4px;
          cursor: pointer; font-size: 12px; color: var(--muted-foreground);
          transition: all 0.2s;
        " onmouseover="this.style.borderColor='var(--foreground)'; this.style.color='var(--foreground)'" onmouseout="this.style.borderColor='var(--border)'; this.style.color='var(--muted-foreground)'">
          Sign Out
        </button>
      </div>
